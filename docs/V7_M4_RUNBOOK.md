...
## 0·补、🔴 收尾的**前置闸门**：不许拿"崩溃残骸"当完整产物

**2026-09-24 事故留下的坑**（由 claude-cf 查出、总管确认）：

run3 在 **16:32 崩溃时留下了 `checkpoints_cifar100/Exp3_FullRobust_resnet18_s43/best_model.pth`
（mtime 16:43:38）**——那是**只跑了 6 分 45 秒**的残骸，不是 200-epoch 的产物。
（崩溃与完成**都会让训练进程退出**，所以"进程没了"**不能**说明"跑完了"。）

### 🔴 闸门（**读出流程的前置条件，不做不许开跑**）

> **对每一个主干：先确认 `outputs_cifar100/extension6_deep_robust/<dir>/metrics.json` 存在，
> 且其 `total_epochs` 等于该 run 的预期 epoch 数（vgg11/exp2 = 120、resnet18/exp3 = 200）。
> 不满足 ⇒ 那个主干的读出流程一律不许启动。**

**为什么必须写死**：读出流程**读的就是那个 `.pth`**。
若不设这道闸门，就会**拿一个 6 分钟的半成品算出"干净漂亮"的数字**——
而那类错误**不会报错**（`load_state_dict` 会因为 shape 一致而静默通过），
**只会产出一个错的结论**。这是本项目最怕的那种失效。

**顺带**：守护进程（`tools/v7_m4_watchdog.ps1`）v2 已把完成判据改成同一条（产物 + `total_epochs`），
**不再把"进程退出"当成"跑完了"**。见该文件头部的 BUG FIXES 段。
