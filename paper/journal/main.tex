...
\textbf{本文只在一族卷积主干（0.25M$\sim$11.2M 参数，以 CIFAR-100 计；全部为 ReLU + 卷积/池化堆叠）上验证过它，
...
四个主干的参数量跨度 0.25M$\sim$11.2M（以 CIFAR-100 计），架构形态全部是 ReLU + 卷积/池化堆叠
（无注意力主干、无归一化层主干），数据集只有 CIFAR-10/100。
...
有效秩损失 @$+0.3$（三主干） & 20.6\% / 33.3\% / 58.7\% & \path{v5_rank_collapse/rank_stats*.csv} \\
秩损失 $\to$ 恢复率（三点，$r$） & $-0.960$ & 同上一行 $+$ \path{v3_readout_repair/} \\
