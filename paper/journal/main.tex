...
\title{\bfseries 逐层非线性失真下神经网络精度退化的读出瓶颈与双杠杆修复}
...
\begin{enumerate}[leftmargin=2em,itemsep=2pt]
  ...
  \item \textbf{修复}：据此得到的读出适配方法（冻结主干、只重训线性读出）在三个主干上
        把尾部提升 13$\sim$23 个百分点，成本低 1$\sim$2 个数量级，
        首次在尾部超过了全网络鲁棒训练这一整类配方。
...
单种子的尾部差值小于 3 个百分点不作为结论（理由见 \cref{sec:variance}）。
...
代价是干净精度下降 3.19$\sim$6.67 个百分点——这一点在 \cref{sec:gating} 里被处理。
...
$\alpha=+0.3$ 从 28.09 提升到 \textbf{42.47}（干净训练）、21.41 到 \textbf{41.55}（鲁棒训练），
