...
与"在哪种分布上训练"无关。**这个对照本来就在数据里**——
`v3_readout_repair` 的产物文件里每个权重都带有多个读出臂，
...
出处：`outputs\_cifar100/v3\_readout\_repair/*ep24*.csv` 的 \texttt{a\_original\_fc}
与 \texttt{b\_clean\_fit\_linear} 两行。）
...
它在 $\alpha=+0.3$ 上更高（52.16 / 58.98 / 47.40），但\textbf{干净精度崩掉}：
