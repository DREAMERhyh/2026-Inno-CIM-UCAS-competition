...
⇒ \textbf{这 5.04 里有多少是"不知道 $\alpha$"的代价、多少是读出训练不足}，
需要一条\textbf{同样用 $\alpha$ 混合数据训练、但按"不知道 $\alpha$"评估}的对照臂才能分开；
本文缺这一臂，因此开放问题第 3 条只能停在"缺口有多大"。
\textbf{下一步}：补该臂（与已有的读出实验同构，只换训练分布）。
