...
即可以约 1/5$\sim$1/30 的主干重训成本在尾部超过所有全网络鲁棒训练配方
...
\textbf{第三行}为各架构上最好的全网络鲁棒训练配方（SimpleCNN 取 v3 路径逐层独立 NAT 的
3 seed 均值 26.57，VGG-11 与 ResNet-18 取 200\,ep 的 Exp3，24.69 / 6.22）。
...
训练时间低约一个数量级（读出 12$\sim$30 min，主干重训 1$\sim$6.5 h）。
...
基准（均匀采样）为 26.03。\textbf{没有一条超过基准}；按本文自订的单种子门槛
...
（架构 $+14.38$、读出 $+15.29$）——\textbf{合起来只拿到六到七成，损失三到四成}。
该组为\textbf{单 seed} 口径（连续均匀 $\alpha$、24\,ep、20k 样本，同 v3 路径）；
