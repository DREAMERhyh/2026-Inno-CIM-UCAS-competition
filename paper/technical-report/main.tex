...
是\textbf{读出的可解码性}而非表征中的信息（探针 headroom $+20.26$ 个百分点，
...
$\alpha=+0.3$ 时探针达 48.35\%、自带头 28.09\%，headroom \textbf{20.26}%
...
与台账 \S16.0。}；
$\alpha=+0.4$ 时 30.90、$+0.5$ 时 30.82。失真越强 headroom 越大。
...
负侧同强度只降 2.0\%。关键在于：\textbf{同强度下正侧的 Fisher 判别比反而高于负侧}——
...
三种非线性读出\textbf{没有一个超过线性头}，差距远超本轮立的可分辨门槛
（$\Delta_{\text{proc}}=3.45$ pp，见下）。而且\textbf{参数越多反而越差}——
典型过拟合序。⇒ 「信息仍线性可达」从"只测过线性"升级为
...
（Adam 50 轮最高 56.31、Adam 400 轮最低 52.85）。
...
\begin{enumerate}[leftmargin=2em,itemsep=2pt]
  \item 在逐层三次多项式非线性失真下，网络在灾难尾损失的是\textbf{读出的可解码性}
        而非表征中的信息（headroom 20.26 个百分点；高斯噪声下仅 7.50）。
