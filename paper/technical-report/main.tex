...
在 CIM 领域，「冻结阵列 + 数字域补偿」已有先例（例如用器件特定的电导漂移查找表配合自适应量化，
在\textbf{不做片上训练}的前提下达到软件基线精度~\cite{mtjcal}），但形态是\textbf{标量或查找表级}校正；
...
主干五个，参数量随类别数变化（见 \cref{tab:arch}）。
...
Wide-CNN$^\dagger$ & 1\,353\,258 / 1\,399\,428 & 4 & 2 & 无 & 秩-恢复第 4 点 \\
...
\vspace{2pt}
{\footnotesize $^\dagger$ 与 SimpleCNN 的差异有\textbf{两个变量}：容量（1.40M 对 0.25M）
与\textbf{第二次下采样位置}（SimpleCNN 的 conv4 在 $16\times16$、本主干在 $8\times8$），
后者是 MACs 约束下的取舍。}
\end{table}
...
七 & 归档后的自主迭代 & 容量非瓶颈（六种读出对照）；headroom 上界抬高；三条排除型判定；判据前提自检回溯审计；\textbf{收官三补}（LP-FT / 秩-恢复第 4 点 / ReLU 非负性） \\
\bottomrule
\end{tabular}
\end{center}
...
\textbf{第二行的 $\pm$ 是"读出种子"口径}（固定该主干、只换 3 个读出种子）；
换成"主干种子"口径（读出侧固定 seed 42、3 个主干种子）后，深层尾部均值为
VGG-11 $46.82\pm0.25$、ResNet-18 $22.91\pm2.61$——
后者低于本表的 $26.82$。两组口径\textbf{只有一格重合}，
是同一张 $3\times3$ 表的两条不同边缘，\textbf{不可相减或互替}。
\end{minipage}
\end{table}
...
前四条独立路径：采样预算分配（三种轮廓，最好 26.13，$+0.10$）、$\alpha$ 空间对抗训练（24.32，$-1.71$）、
...
预算分配的 $+0.10$ 与基准同级。\textbf{前四条都没有改善尾部。}

最强的一条证据是范围外推：\textbf{模型已在 $\pm0.5$ 上训练过}，
测试 $\alpha=+0.5$ 时仍只有 2.71\%，同期负侧 $-0.5$ 有 31.19\%（差 28.5 个百分点）。
正侧尾部不是采样覆盖问题，而是\textbf{结构性的不可训练区}。

\textbf{第五条动的是主干本身：先训读出、再解冻微调（LP-FT）。}
前四条都在"冻结主干、只改采样分布"的框架内。第五条先按上文读出适配把线性头训好（LP），
再\textbf{解冻主干}与该头联合微调（FT）——这是分布偏移场景下被证明优于全量微调的配方
（~\cite{lpft}；原理是先把头训好再解冻，避免随机初始化的头在早期用大梯度破坏预训练特征）。
判据在跑前写死：$\alpha=+0.3$ 上相对\textbf{同一次运行内}的纯读出适配提升 $\ge3$\,pp 即成立。
结果如表 \ref{tab:lpft}：\textbf{三个主干一致大幅下降——不是"没有提升"，而是"净损伤"。}
...
LP 24\,ep / lr $10^{-2}$，FT 30\,ep / lr $10^{-3}$。数据：\path{outputs_cifar100/v7_m8c_lpft/}。}
\end{table}

\textbf{机制比数字更有信息量}：FT 阶段\textbf{训练精度一路上涨}
（SimpleCNN 88.9\%、VGG-11 96.9\%，在混合 $\alpha$ 的训练分布上），
说明模型在训练分布上确实变好了；而\textbf{各 $\alpha$ 的测试读数全线变差、
且尾部降幅远大于干净精度降幅}（VGG-11：$-33.83$ 对 $-10.38$）。
⇒ \textbf{在失真分布上微调破坏了预训练特征}——这正是 LP-FT 论文在分布偏移场景下要避免、
而在失真场景下没有被避免的现象。
⇒ \textbf{它支持"训练端做的事越少越好"，不构成对既有结论的修正}；
同时把前四条的"无效"升级为"动到主干会主动变坏"。
...
\caption{秩损失与读出可恢复量（四点，$r=-0.934$）。
恢复率 = 读出适配 @$\alpha=+0.3$ 的精度 $\div$ 同一文件里原始头 @$\alpha=0$ 的精度。
四点\textbf{已不是单调关系}：最左两点秩损失只差 0.7 个百分点，恢复率却差 4.0 个百分点。
...
反向不成立——ResNet-18 掉了 58.7\% 的秩，读出仍拿回 36.9\%。
...
⇒ 按预登记判据，\textbf{四点仍不足以判定曲线形状}——"先平后塌"仍只是提示。

\textbf{补测顺带得到一条比判据本身更重要的发现}：最左两点秩损失只差 \textbf{0.7} 个百分点，
恢复率却相差 \textbf{4.0} 个百分点（$72.2-68.2$），\textbf{超过本文自订的 3 个百分点分辨门槛}，
且两点出自同一读出流程、同一秩统计口径、同为 3 seed。
⇒ \textbf{秩损失不是恢复率的唯一决定因素}，存在秩损失之外的变量。
...
\textbf{机制：保护主要来自降维。} 四个候选解释被逐一检验，其中三个被排除、一个判为不可分辨：
...
\textbf{ReLU 的非负性} & 换成 GELU（破坏非负性）后保护是否变弱
& $P(\text{ReLU})=+11.81$ 对 $P(\text{GELU})=+14.71$（SimpleCNN，
  两种池化密度 $\times$ 3 seed），差 $-2.91$，落在预登记 $\pm4$ 带内，
  \textbf{方向还随种子翻转} ⇒ \textbf{不可分辨} \\
\bottomrule
\end{tabular}
\end{table}

\textbf{第四条（ReLU 非负性）是唯一没被排除的，但也没能证实。}
\cref{tab:pool} 的前三条都没有排掉一个更基本的原因：ReLU 把池化输入限制在\textbf{非负域}，
而在那里归一化三次映射与 max 精确可交换（第一行实测）。
若保护来源于这个非负性，\textbf{削弱非负性的激活应当让保护变弱}。
实测（SimpleCNN，$2\times$ 与 $5\times$ 两种池化密度 $\times$ 两种激活，每格 3 seed）：
池化保护 $P=(\text{2-pool 的 drop})-(\text{5-pool 的 drop})$，
得 $P(\text{ReLU})=+11.81$、$P(\text{GELU})=+14.71$，差 $-2.91$，
落在预登记 $\pm4$（$2\sigma$）带内 ⇒ \textbf{判为不可分辨，既不判成立也不判否证}。
更关键的是\textbf{方向本身不稳定}：单 seed 时 $+2.57$（与假说同向），3 seed 时 $-2.91$（\textbf{反向}）。
\textbf{这不是"接近显著"，而是"这个量级的效应在本协议下判不了"}——
其噪声主要来自 $2\times$max 一侧（种子标准差 $3.03$ / $2.89$，
而 $5\times$max 一侧只有 $1.54$ / $0.26$，\textbf{相差一个量级}）。
...
\textbf{若还要继续追这两条，预算应花在 $2\times$max 的更多种子上}
（$5\times$max 一侧的种子方差已经小一个量级，加种子收益低）。
...
上面那条"跨数据集未复现"是经验事实；它后来有了一个\textbf{机制级}的解释，
...
（$\phi=0$ 即相互独立）。实测（7 个 $\alpha$ 点 $\times$ 3 个种子 $=$ 每个数据集 21 格；
下面的范围是\textbf{按 $\alpha$ 点做 3 seed 平均后}的逐点极值）：
$\phi$ 的范围在 CIFAR-100 上是 $+0.594\sim+0.706$、CIFAR-10 上是 $+0.519\sim+0.830$；
"两头都错"的格数相对独立基线为 $1.50\sim1.98$ 倍与 $2.76\sim6.58$ 倍。
⇒ \textbf{$\phi$ 在两个数据集上都是强正}——两个头的错误\textbf{正相关}，它们在同一批样本上\textbf{一起错}，
\textbf{「插值即集成」所依赖的互补性在两个数据集上都不存在}。

由此两条：\textbf{①} 那个机制是被\textbf{否证}，不是"未能分辨"——
本项目因此把「插值即集成」\textbf{永久关闭}；
\textbf{②} 收益的上界由此可解释：没有互补性可利用，插值的增益注定有限
（实测"软门控减 oracle"的极差，CIFAR-100 为 $1.60$ pp、CIFAR-10 只有 $0.71$ pp）。
但要分清：\textbf{被关闭的是机制解释，不是那个效应本身}——
CIFAR-100 上软门控确有若干点超过两个头，那个效应仍然存在、也仍未被完全解释；
本段只说明了\textbf{它的上界为什么不高}，没有说明\textbf{它为什么不为零}。
...
& 否证：三种子来自两脚本；同协议标准差只有 $\pm0.39$，混协议把标准差估高约 6 倍 \\[2pt]
2 & $\alpha$ 采样\textbf{粒度}
...
\begin{enumerate}[leftmargin=2em,itemsep=2pt]
  ...
  \item \textbf{散布对比三要求}：同脚本 / 已交叉评估 / 未校准的组必须标注。
...
末尾补记收官阶段的三项实验（LP-FT / 秩-恢复第 4 点 / ReLU 非负性）。
...
神经坍缩（NC）几何 & \textbf{未能构成稳健排除}。类均值夹角偏离与模长变异系数
  在\textbf{以全局均值为中心}的口径下正侧 5/5 上升（即\textbf{远离} ETF）；
  但换成\textbf{不做中心化}的口径，模长变异系数只有 1/5 上升——
  \textbf{两版给出不同的判决分支}。⇒ 该几何量对预处理敏感，
  「正侧 Fisher 反而高于负侧」这条异常\textbf{未能由它稳健地排除} \\
...
\textbf{第二条（NC 几何）在本轮审计后不再作排除用}：它两版判决不同、对预处理敏感，
此处如实记为"未能构成稳健排除"——\textbf{这条降级本身就是信息}：
它说明该几何量在本配置下不具备判决力，因此不能拿它去否证别的解释。
...
层级 & 变的是什么 & 实测幅度 \\
\midrule
① 种子噪声 & 同一过程重复 & 尾部 $\pm0.25\sim3.40$；clean $\pm0.07\sim1.85$ \\
...
\subsection{收官补测：三项实验（2026-09-26）}

M4 收工之后还有一段 GPU 空窗，补跑了三项此前排在队尾的实验。
\textbf{三项都不需要修改既有结论}，其中一项把已有的表述改得更准。
详细台账见 §16.9 / §16.10 / §16.11，正文细节已并入上文对应各节。

\begin{center}
\small
\begin{tabular}{p{2.3cm}p{2.5cm}p{8.9cm}}
\toprule
实验 & 判决 & 一句话 \\
\midrule
\textbf{M8c} LP-FT
& \textbf{阴性（强）}
& 先训读出、再解冻主干微调，\textbf{三个主干一致大幅下降}
  （$-11.85$ / $-33.83$ / $-12.88$）。机制：FT 阶段训练精度涨到 88.9\% / 96.9\%，
  而各 $\alpha$ 测试全线变差、尾部降幅远大于干净降幅
  ⇒ \textbf{微调破坏了预训练特征}。这正是 LP-FT 论文在分布偏移下要避免的现象，
  \textbf{在失真场景下没有被避免}。⇒ 支持"训练端做的事越少越好" \\
\textbf{M5} 秩-恢复第 4 点
& \textbf{仍不足以判定形状}
& 新增 Wide-CNN 容量档（1.40M 参数 / 133.3M MACs），干净精度 65.97\% 居中，
  \textbf{但秩损失 19.9\% 与 SimpleCNN 的 20.6\% 几乎重合}，没有落进中间段。
  \textbf{顺带发现比判据更重要}：秩损失只差 0.7 pp、恢复率却差 4.0 pp
  ⇒ \textbf{秩损失不是恢复率的唯一决定因素} \\
\textbf{M14b} ReLU 非负性
& \textbf{不可分辨}
& 把 ReLU 换成 GELU：$P(\text{ReLU})=+11.81$、$P(\text{GELU})=+14.71$，
  差 $-2.91$，落在预登记 $\pm4$ 带内。
  \textbf{方向还随种子翻转}（单 seed $+2.57$、3 seed $-2.91$）
  ⇒ \textbf{不是"接近显著"，是"判不了"} \\
\bottomrule
\end{tabular}
\end{center}

\textbf{这一轮的附带产出（比三条判决本身更值得记）}：

\begin{enumerate}[leftmargin=2em,itemsep=2pt]
  \item \textbf{一个新的、可执行的下一个问题}。M5 把"秩损失预测可恢复量"这条
        从一个\textbf{开放搜索}变成了一次\textbf{有对照的检验}——
        我们已知道两个主干在几乎相同的秩损失下相差 4.0 pp，
        那么找出它们\textbf{除秩损失之外还差什么}，答案就在里面。
  \item \textbf{噪声来源被定位}。M14b 补 seed 后看出：$2\times$max 的种子标准差
        （$3.03$ / $2.89$）比 $5\times$max（$1.54$ / $0.26$）\textbf{大一个量级}。
        ⇒ 若要真正裁决非负性假说，预算应花在 $2\times$max 的更多种子上。
  \item \textbf{一条负面方法学经验}。全模型反传叠加"每 batch 注册失真 hook"
        会让训练慢 7 倍（实测 $>13$\,min/epoch，而该主干从头训练是 105 s/epoch），
        实验一度被迫中止。⇒ 微调类实验要单独配 batch 与 hook 策略，
        不要复用"冻结主干前向 / 浅层训练"的配置。
\end{enumerate}
...
& 阵列里的权重是烧录/固化的；而且训练端五条路本来就救不了尾部 \\
\bottomrule
\end{tabular}
\end{table}
...
原写"深层主干是单次训练产物……涉及深层的所有差值是否在主干种子方差内无法裁决"。
...
读出适配线上的深层差值因此可以裁决（须写明在已测主干上成立）；
而门控红利 $-1.14$ 仍落在主干级散布 $2.61$ 之内，\textbf{这一条仍然不可裁决}——
理由从"没有测"变成了"测了，方差确实盖得住它"。

\textbf{(3) 池化保护的机制分解没有定下来。}
两个子问题都没有结论：\textbf{① 定量分解}——avg 与 max 的差值只有 $+1.17$，$z\approx1.1$；
\textbf{② 非负性}——换成 GELU 后保护之差为 $-2.91$，落在预登记 $\pm4$ 带内，
\textbf{且方向随种子翻转}（单 seed $+2.57$、3 seed $-2.91$），只能记为不可分辨。

\textbf{(4) 秩-恢复关系仍不足以定形状，且秩损失不是唯一决定因素。}
第 4 个容量档已补齐，但它与 SimpleCNN 的秩损失几乎重合（19.9\% 对 20.6\%），
没有落进中间段，故曲线形状仍未判定；同时它暴露出\textbf{秩损失相同的主干恢复率可差 4.0 个百分点}
（$>3$ pp 分辨门槛）⇒ \textbf{秩损失之外还有一个变量，本文未识别}。
...
（\path{outputs_cifar100/extension5_dithering_vgg11/arch_dithering_summary.csv}）——
...
一条用 $\alpha$ 混合数据训练的（与部署同构的）读出臂\textbf{尚未产出}（脚本与预登记判据已就绪，受资源限制未执行）；
...
6 & 深层主干的重训散布 & 4 次训练 $\times$ 4h & 贵，但能说清深层结论的置信度 \\
...
& $\ge2$ 次训练 \\
深层主干散布 & \textbf{已兑现（M4）}：主干级标准差 VGG-11 $0.25$、ResNet-18 $2.61$，
  均小于读出级，深层差值因此可以裁决 & 已收工 \\
池化保护的两成分分解 & 每组 $\ge8$ 个种子，\textbf{或}换效应量更大的对照（池化窗口 $2\times2$ vs $4\times4$）。
  \textbf{补充（M14b 实测）}：同一批数据还带出一条预算指引——
  噪声集中在 $2\times$max 一侧（种子标准差差一个量级），追加种子应投在那里 & 9$\sim$10 h \\
秩-恢复曲线形状 & \textbf{已测（M5）}：第 4 点与 SimpleCNN 的秩损失几乎重合（19.9\% 对 20.6\%），
  故\textbf{形状仍定不了}，判为"仍不足以判定"。
  \textbf{但这次补测引出一个更值得做的问题}——找出秩损失之外那个变量 & 已收工 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}[leftmargin=2em,itemsep=2pt]
  ...
  \item 先训读出、再解冻主干微调（LP-FT）——\textbf{不但无效，三个主干一致大幅下降}
        （$-11.85$ / $-33.83$ / $-12.88$），机制是微调破坏预训练特征；
...
\begin{enumerate}[leftmargin=2em,itemsep=2pt]
  ...
  \item 据此，冻结全部模拟权重、只重训最后的线性层，可把尾部提升 \textbf{$+15.2\sim+31.8$} 个百分点（CIFAR-100，24\,ep/20k，各 3 seed），
        ...
        并一致超过所有全网络鲁棒训练配方。
  \item 训练端的五条补救路径没有一条改善尾部（前四条中两条显著劣于基准、一条落在单种子门槛内、
        一条与基准同级；\textbf{第五条解冻主干微调反而主动破坏}，三主干 $-11.85$/$-33.83$/$-12.88$）；
        正侧尾部是结构性不可训练区，
