...
主要结果如下。\textbf{任务1}：正向失真（$\alpha>0$）的破坏力是负向的 3.4 倍，
误差从浅层向深层累积，残差连接是误差放大器——$\alpha=+0.3$ 时 ResNet-18 从 92.09\% 掉到 17.08\%，
跌幅 75 个百分点，显著重于 VGG-11（36.70\%）与 SimpleCNN（55.08\%）。
...
全部数值均由脚本从实验产物文件现读，每个实验的强度扫描文件里 $\alpha=0$ 的行
必须等于同目录训练指标文件记录的最优测试精度（容差 0.01），这个自检覆盖了本文引用的每一行数据。
...
破坏力的分布极不对称：$\alpha=+0.3$ 时三个模型分别跌 29.82、53.10、75.01 个百分点，
而 $\alpha=-0.3$ 时的跌幅小得多。
...
正向失真（$\alpha>0$）的破坏力显著大于负向，且跌幅随深度与残差连接递增。
全量测试集、best 权重、纯推理。数据：\texttt{outputs/task1\_*/alpha\_sensitivity.csv}。}
\label{fig:c01}
\end{figure}

在 SimpleCNN 上，正向与负向的破坏力之比为 \textbf{3.4}（以 $|\alpha|=0.3$ 的跌幅计）。
这个比值是后续所有训练策略与采样设计的动机来源。
...
微调策略的外推衰减最缓（$\alpha=+0.5$ 处仍为 $+1.08$），
是三种权重中唯一在深度外推区不恶化的。
...
同时还得到一个配方设计的警示：3 bit 的次加性幅度比 4 bit 更大，
但绝对精度已经腰斩（10.87\% vs 19.75\%）。
...
\begin{enumerate}[leftmargin=2em,itemsep=2pt]
  \item 正向失真的破坏力是负向的 3.4 倍，误差从浅层向深层超加性累积；
        残差连接是误差放大器，ResNet-18 在 $\alpha=+0.3$ 下损失 75 个百分点。
  ...
  \item 非线性失真与高斯噪声的鲁棒性\textbf{不可跨域迁移}，
...
\textbf{局限}：三项拓展的数据集多样性（评审建议 R4）在时间约束下未执行；
